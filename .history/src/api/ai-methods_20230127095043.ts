...
export const generateSacarsticSentence = async (prompt: string) => {
    const response = await openai.createCompletion({
        model: "text-davinci-003",
        prompt: `Gere uma frase sacartisca com os seguintes dados

prompt: "A pink flower falling from the sky, dynamic art, souvenir, brush art"
frase: Uma flor? Tudo bem né? aff calma estou fazendo sua arte.

prompt: "A small apple lying on a lawn next to a tree, digital art, brush art, done by Da Vince"
frase: Alo 📞! Eu não ja disse que esrou fazendo? Aff
